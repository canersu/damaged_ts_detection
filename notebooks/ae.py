...
class autoEncoder:
    ...
    def dataset_all(self, dsPath, imgHeight, imgWidth, imgChannel, valSize):
        self.img_shape = (imgHeight, imgWidth, imgChannel)
        for i in range(43):
            ts_db = os.path.join(dsPath+str(i)+'/')
            for img in os.listdir(dsPath+str(i)+'/'):
                image = Image.open(os.path.join(ts_db, img))
                image = image.resize((imgHeight,imgWidth))
                image = np.array(image)
                image = image/255.0
    ...
    # TODO: Add draw graph method for comparison
